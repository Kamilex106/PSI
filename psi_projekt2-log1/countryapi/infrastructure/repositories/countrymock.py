...
class CountryMockRepository(ICountryRepository):
    ...
    async def update_country(
        ...
        if country_pos := next(
                (index for index, country in enumerate(countries) if country.id == country_id),
                None
        ):
            countries[country_pos] = Country(id=country_id, **data.model_dump())

            return countries[country_pos]
